  ...
  <div v-if="data.openModal" class="fixed top-0 left-0 right-0 bottom-0 backdrop-blur-sm bg-[rgb(0,0,0,0.2)] cursor-pointer z-[999]" @click="data.openModal = !data.openModal">
    <div @click.stop class="cursor-auto absolute top-[50%] left-[50%] translate-y-[-50%] translate-x-[-50%] rounded-2xl w-[80%] p-5 pb-20 lg:p-10 lg:pb-24 lg:w-[400px]" :class="data.completed ? 'bg-[#EDB046]' : 'bg-white'">
      <div v-if="!data.updateTaskModel">
        <h1 class="font-bold text-[16px] lg:text-[28px] text-center mb-1 lg:mb-3">{{data.title}}</h1>
        <p class="font-regular text-[#BA5112] text-[12px] lg:text-[16px] text-center mb-3 lg:mb-7">added 3d ago</p>
