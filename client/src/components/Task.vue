<template>
	<div class="flex w-full border-2 min-h-[70px] p-5 cursor-pointer" :class="task.completed ? 'bg-[#EDB046]' : 'bg-white'" @click="data.openModal = !data.openModal">
		<div class="mr-5" @click.stop>
			<svg v-if="!task.completed" class="lg:w-[28px] lg:h-[28px] transition-all hover:scale-105" viewBox="0 0 28 28" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      <svg v-else class="lg:w-[23px] lg:h-[23px] transition-all hover:scale-105 mt-1" viewBox="0 0 22 22" fill="none" xmlns="http://www.w3.org/2000/svg">
		...
		<div class="font-regular text-[20px]">
			<p>
        {{task.title}}
      </p>
		</div>
